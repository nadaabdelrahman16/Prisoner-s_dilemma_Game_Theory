...
class P2():
    ...
    def weakly_Player2(self,weaklyMatrix,weaklyP2_action):
        #Player2
        ...
        arr_size = np.size(arr, 0)
        while (i < n):
            # print(weaklyMatrix[i][c1[0]][1])
            # print(weaklyMatrix[i][c1[1]][1])
            if (weaklyMatrix[i][c1[0]][1] >= weaklyMatrix[i][c1[1]][1]):
                i += 1
                if (i == n):
                    no_delete = set()
                    weaklyMatrix = np.delete(weaklyMatrix, c1[1], 1)
                    weaklyP2_action=np.delete(weaklyP2_action,c1[1])
                    no_delete.add(str(c1[1]))
                    perm = [x for x in perm if not no_delete & set(str(x))]
                    arr_size -= 1
                    m-=1
                ...
                if (len(perm) == 0):
                    ...
                    c1 = perm[r]
                    i= 0
        print("p2", weaklyMatrix)
        return weaklyMatrix,weaklyP2_action